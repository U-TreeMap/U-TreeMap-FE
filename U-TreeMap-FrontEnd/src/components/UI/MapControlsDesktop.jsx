...
export default function MapControlsDesktop({ map }) {
  if (!map) return null;
  ...
  <div>
    <div className="absolute bottom-7 right-4 z-10 flex flex-col gap-[10px]">
      ...
      <div
        ...
        className={`${buttonBase} ${ desktop_rocate } bg-white ${shadowNormal} ${shadowHover}`}
      >
        <img
          src={target}
          className={`${ desktop_mod} pointer-events-none`}
        />
      </div>

      {/* 🔍 줌 버튼 박스 */}
      <div className={ desktopZoomBox}>

        {/* ➖ Zoom Out */}
        <div
          onClick={() => zoomOut(map)}
          className={`${buttonBase} ${desktop_mod}`}
        >
          <img
            src={minus}
            className={`${ desktop_mod} pointer-events-none [filter:drop-shadow(2px_2px_6px_rgba(0,0,0,0.25))]`}
          />
        </div>
